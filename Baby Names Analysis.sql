...
SELECT                                             -- This query count the number of distinct names per gender and get the total names (without looking at the
	Gender,                                        -- gender).
...
SELECT                                                 -- This query filters the data by male gender, then groups the observations by name to calculate the total
	Name, SUM(Births) AS Total_babies                  -- number of births. Finally, the result is sorted in descending order by total babies born and only the
FROM names                                             -- first row is returned.
WHERE Gender = "M"
GROUP BY Name
...
(SELECT                                                -- aggregate the total births. The second CTE, adds a column to the previous CTE to show the popularity
	Name,                                              -- using the ROW_NUMBER function. Finally, the rows whose first name is not Jessica are filtered out to.
	Year,                                              -- show the popularity of that name per year
    SUM(Births) AS Total_babies
FROM names
WHERE Gender = "F"
GROUP BY
...
SELECT                                       -- This query calculates the number of births per region by doing a LEFT JOIN between a modified version of the
	Region,                                  -- regions table and the names table.
    SUM(Births) AS Total_babies
FROM
(SELECT
	CASE
...
WITH long_names AS                           -- This query starts by grouping the names to add the total babies who received a particular name and the length of
...
(SELECT										-- received the name Chris per region. Then, an INNER JOIN is used to calculate the percentage of babies called Chris
	State,									-- per state.
    SUM(Births) AS Total_babies
FROM names
GROUP BY State),
